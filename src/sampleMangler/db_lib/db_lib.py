...
def createContainer(container_name,type_name,capacity):
  containerObj = {"container_id":int(time.time()), "containerName":container_name,"type_name":type_name, "item_list":[]}
#the item list is a list of id's, whether they be other containers or samples. This is becasue samples and pucks can move.
...
def createDefaultRequest(sample_id):
  request = {"request_id": int(time.time()), "sample_id" : sample_id,"sweep_start":0.0, "sweep_end":1.0, "img_width":.1, "exposure_time":.1, "priority":0,"protocol":"standard","directory":"/","file_prefix":getSampleNamebyID(sample_id)+"_data","file_number_start":1,"wavelength":1.1,"resolution":3.0,"slit_height":100.0,"slit_width":150.0,"attenuation":0,"pos_x":0,"pos_y":0,"pos_z":0,"pos_type":'A',"gridW":0,"gridH":0,"gridStep":0}
  return request
...
def insertCollectionRequest(sample_id, sweep_start, sweep_end, img_width, exposure_time, priority,protocol,directory,file_prefix,file_number_start,wavelength,resolution,slit_height,slit_width,attenuation,pos_x,pos_y,pos_z,pos_type,gridW,gridH,gridStep):

  colobj = {"request_id": int(time.time()), "sample_id":sample_id, "sweep_start":sweep_start, "sweep_end":sweep_end, "img_width":img_width, "exposure_time":exposure_time, "priority":priority,"protocol":protocol,"directory":directory,"file_prefix":file_prefix,"file_number_start":file_number_start,"wavelength":wavelength,"resolution":resolution,"slit_height":slit_height,"slit_width":slit_width,"attenuation":attenuation,"pos_x":pos_x,"pos_y":pos_y,"pos_z":pos_z,"pos_type":pos_type,"gridW":gridW,"gridH":gridH,"gridStep":gridStep}
